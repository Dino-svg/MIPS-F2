...
# Mapa de instrucciones ampliado
inst_map = {
    # Tipo R
...
def preprocess_asm(lines):
    ...
    for line in lines:
        ...
        # Manejar etiquetas
        if ':' in line:
            label, instruction = line.split(':')[0], line.split(':')[1].strip()
...
def convert_asm_to_bin(asm_line, current_addr, labels):
    ...
    parts = [p.replace(',', '') for p in asm_line.split()]
    inst = parts[0].upper()
    ...
    try:
        if inst_map[inst]["type"] == "R":
            ...
            if inst == "J":
                target = f"{labels[parts[1]] >> 2:026b}"
                return f"{inst_map[inst]['opcode']}{target}"
...
# Interfaz gráfica
root = tk.Tk()
root.title("Conversor ASM a Binario - MIPS")
